...
async function processCall(session) {
  ...
  sb.logCallEnd(callSid, {
    ...
    urgency: isEmergency ? 'emergency' : (grade === 'HOT' ? 'high' : grade === 'WARM' ? 'normal' : 'low'),
    grade: grade,
    outcome: asText(ex.outcome, 300)
  });
  sb.logEvent(callSid, 'voice_graded', 'Caller graded ' + grade + ': ' + (ex.grade_reason || ''),
    { grade: grade, reason: ex.grade_reason, intents: session.intents, emergency: isEmergency });
