...
// Day 9: a caller said STOP on the phone. opted_out_at is what sr_gate reads
// on every future send, so this one write silences every machine for them.
async function markOptedOut(phoneE164) {
if (!phoneE164) return false;
try {
const res = await fetch(SUPABASE_URL + '/rest/v1/contacts?tenant_id=eq.' + TENANT_ID +
'&phone_e164=eq.' + encodeURIComponent(phoneE164), {
method: 'PATCH',
headers: { ...HEADERS, Prefer: 'return=minimal' },
body: JSON.stringify({ opted_out_at: new Date().toISOString() })
});
if (!res.ok) console.error('[supabase] opt-out mark -> ' + res.status);
return res.ok;
} catch (e) {
console.error('[supabase] opt-out mark error: ' + e.message);
return false;
}
}
...
requestApproval, insertLead, markOptedOut
};
